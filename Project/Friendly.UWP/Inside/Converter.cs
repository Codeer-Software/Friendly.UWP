...
namespace Friendly.UWP.Inside
{
    class Converter
    {
        ...
        {
            return new Friendly.Core.ProtocolInfo(Convert(src.ProtocolType),
        ...
        private static Friendly.Core.VarAddress Convert(Codeer.Friendly.Inside.Protocol.VarAddress src)
        {
            return src == null ? null : new Friendly.Core.VarAddress(src.Core);
        }

        private static Friendly.Core.OperationTypeInfo Convert(Codeer.Friendly.OperationTypeInfo src)
        {
            return src == null ? null : new Friendly.Core.OperationTypeInfo(src.Target, src.Arguments);
        }

        private static Friendly.Core.ProtocolType Convert(Codeer.Friendly.Inside.Protocol.ProtocolType src)
        {
            switch (src)
            {
        ...
        private static Codeer.Friendly.Inside.Protocol.VarAddress Convert(Friendly.Core.VarAddress returnValue)
        {
            return new Codeer.Friendly.Inside.Protocol.VarAddress(returnValue.Core);
        }

        private static Codeer.Friendly.Inside.Protocol.ExceptionInfo Convert(Friendly.Core.ExceptionInfo src)
        {
            try
            {
                throw new NotSupportedException();
